...
export const ArticleCard = ({
  ...
  return (
    <Link href={`/article/${article.slug}`} className="block h-full" prefetch={false}>
      <div
        key={article.id}
        className={cn(
            ...
            {article.tags && article.tags.length >= 2 ? (
              <Badge className="px-2 py-1 break-all">
                <span className="flex flex-row items-center gap-1">
                  <span className="text-white text-xs font-bold leading-none mr-1">+{article.tags.length}</span>
                  {article.tags.map((tag, idx) => (
                    <span
                      key={tag.id}
                      className={`inline-block w-[15px] h-[15px] rounded-xs overflow-hidden bg-white/10 align-middle${idx !== 0 ? ' -ml-3' : ''}`}
                    >
                      {tag.icon?.url && (
                        <Image
